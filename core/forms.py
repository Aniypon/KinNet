...
def local_date_input():
    return forms.DateInput(format="%d.%m.%Y", attrs=DATE_INPUT_ATTRS.copy())
...
class EventForm(forms.ModelForm):
    class Meta:
        model = Event
        fields = ["title", "date", "kind", "recurrence", "member", "description", "remind_days_before", "tags"]
        widgets = {
            "date": local_date_input(),
        }
        labels = {
            "title": "Название",
            "date": "Дата",
    ...
    def __init__(self, *args, family=None, **kwargs):
        ...
        self.fields["member"].empty_label = "Без привязки"
        self.fields["tags"].queryset = Tag.objects.filter(kind="event")
        if self.instance and self.instance.kind == "birthday":
            self.fields["recurrence"].initial = "yearly"

    def clean(self):
        cleaned = super().clean()
        if cleaned.get("kind") == "birthday":
            cleaned["recurrence"] = "yearly"
        return cleaned
...
class MessageForm(forms.ModelForm):
    class Meta:
        ...
        fields = ["text"]
        widgets = {
            "text": forms.Textarea(attrs={"rows": 2, "placeholder": "Напишите сообщение семье..."}),
        }
        labels = {
            "text": "Сообщение",
        }
...
class FamilyPhotoForm(forms.Form):
    ...
    event = forms.ModelChoiceField(
        ...
        required=False,
        help_text="Можно оставить пустым, если фото не относится к конкретному событию.",
    )

    def __init__(self, *args, family=None, **kwargs):
        super().__init__(*args, **kwargs)
        if family is not None:
            self.fields["event"].queryset = family.events.order_by("-date", "title")
